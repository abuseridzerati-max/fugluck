...
export class RunnerEngine {
    ...
    update(dtSec, input) {
        ...
        const baseSpawnIntervalMs = Math.max(WORLD.minSpawnIntervalMs, WORLD.initialSpawnIntervalMs - this.elapsed * WORLD.spawnIntervalDecayPerSec);
        ...
        if (this.spawnTimerMs <= 0) {
            ...
            const type = this.gameplayRng() < 0.5 ? "hurdle" : "overhang";
    ...
    draw(ctx) {
        ...
        if (this.frameCount < 5) {
            ...
            console.log(`[neon-runner] DIAGNOSTIC draw#${this.frameCount}: engineWH=${width}x${height} groundY=${groundY} ` +
        ...
        if (this.frameCount < 5) {
            // TEMPORARY DIAGNOSTIC — session 21, Neon Runner baseline. Remove
            // once the Sky Dodge bug is confirmed fixed (see PROGRESS.md).
            console.log(`[neon-runner] DIAGNOSTIC draw#${this.frameCount} player rect: ` +
