...
async function main(){
 ...
 const request=(method:string,suffix='',data?:unknown,user:string|null=owner)=>fetch(base+suffix,{method,headers:{'Content-Type':'application/json',...(user?{Cookie:`${ADMIN_SESSION_COOKIE_NAME}=${signSessionToken({sub:user})}`}:{})},body:data===undefined?undefined:JSON.stringify(data)});
  ...
  check('owner can grant positive integer TEST GEL',grantResponse.status===200&&grant.success&&grant.availableMinor===12345&&grant.accountingReferenceId?.startsWith('sar_test_funding_')&&grant.auditLogId);
  const grantLedger=await pool.query('SELECT amount_minor,balance_type,event_type FROM sandbox_ledger_entries WHERE accounting_reference_id=$1 ORDER BY amount_minor',[grant.accountingReferenceId]);
  check('TEST funding uses balanced append-only double entry',grantLedger.rows.length===2&&grantLedger.rows.reduce((n,r)=>n+Number(r.amount_minor),0)===0&&grantLedger.rows.every(r=>r.event_type==='TEST_FUNDING_GRANT')&&grantLedger.rows.some(r=>r.balance_type==='SETTLED')&&grantLedger.rows.some(r=>r.balance_type==='AVAILABLE'));
  const grantAudit=await pool.query('SELECT action,admin_user_id,target_id,amount,currency,reason,details FROM admin_audit_logs WHERE id=$1',[grant.auditLogId]);
  const auditRow=grantAudit.rows[0];
  check('audit record is linked to the accounting reference',auditRow?.action==='ADMIN_COMPETITION_GRANT_TEST_FUNDS'&&auditRow.admin_user_id===owner&&auditRow.target_id===player&&Number(auditRow.amount)===12345&&auditRow.currency==='GEL'&&auditRow.reason==='Phase 5.5A staging QA funding'&&auditRow.details.accountingReferenceId===grant.accountingReferenceId&&auditRow.details.newBalanceMinor===12345);
  check('grant history returns user, admin, reason, and accounting reference',(await adminRequest(`/competitions/accounting/grants?query=${encodeURIComponent(player)}`).then(r=>r.json()) as any).grants?.some((g:any)=>g.targetUserId===player&&g.adminUserId===owner&&g.details?.accountingReferenceId===grant.accountingReferenceId));
  check('missing grant reason rejected',(await adminRequest('/competitions/accounting/grant','POST',{targetUserId:player,amountMinor:100})).status===400);
  check('fractional minor-unit grant rejected',(await adminRequest('/competitions/accounting/grant','POST',{targetUserId:player,amountMinor:12.5,reason:'invalid amount test'})).status===400);
  check('non-admin cannot grant test funds',(await adminRequest('/competitions/accounting/grant','POST',{targetUserId:player,amountMinor:100,reason:'permission test'},player)).status===403);
  const userDetail=await adminRequest(`/users/${player}`).then(r=>r.json()) as any;
  check('user detail returns updated TEST GEL and accounting activity',userDetail.user?.sandboxBalances.availableMinor===12345&&userDetail.recentSandboxLedger.length===2&&Array.isArray(userDetail.recentCompetitions));
