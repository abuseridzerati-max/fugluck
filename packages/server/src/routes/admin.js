...
import { ADMIN_SESSION_COOKIE_NAME, requireOwnerAdmin } from "../auth/middleware";
...
adminRouter.get("/dashboard", requirePermission("ADMIN_VIEW_AUDIT"), async (_req, res) => {
    ...
    const [coinsCirculationRes] = await db.select({ value: sql `coalesce(sum(amount), 0)::int` }).from(ledgerEntries).where(eq(ledgerEntries.currency, "COINS"));
    const [diamondsCirculationRes] = await db.select({ value: sql `coalesce(sum(amount), 0)::int` }).from(ledgerEntries).where(eq(ledgerEntries.currency, "DIAMONDS"));
...
adminRouter.get("/users", requirePermission("USERS_VIEW"), async (req, res) => {
    ...
    const userResults = await Promise.all(userList.map(async (u) => {
        const [coinsRes] = await db.select({ value: sql `coalesce(sum(amount), 0)::int` }).from(ledgerEntries).where(and(eq(ledgerEntries.userId, u.id), eq(ledgerEntries.currency, "COINS")));
        const [diamondsRes] = await db.select({ value: sql `coalesce(sum(amount), 0)::int` }).from(ledgerEntries).where(and(eq(ledgerEntries.userId, u.id), eq(ledgerEntries.currency, "DIAMONDS")));
        return {
            id: u.id,
            username: u.username,
            email: u.email,
...
adminRouter.get("/users/:id", requirePermission("USERS_VIEW"), async (req, res) => {
    ...
    const [coinsRes] = await db.select({ value: sql `coalesce(sum(amount), 0)::int` }).from(ledgerEntries).where(and(eq(ledgerEntries.userId, targetId), eq(ledgerEntries.currency, "COINS")));
    const [diamondsRes] = await db.select({ value: sql `coalesce(sum(amount), 0)::int` }).from(ledgerEntries).where(and(eq(ledgerEntries.userId, targetId), eq(ledgerEntries.currency, "DIAMONDS")));
...
adminRouter.get("/matches", requirePermission("MATCHES_VIEW"), async (req, res) => {
    ...
    const conditions = [];
    if (matchId.length > 0)
        conditions.push(eq(matchesHistory.id, matchId));
    if (gameId.length > 0)
        conditions.push(eq(matchesHistory.gameId, gameId));
    if (status.length > 0)
        conditions.push(eq(matchesHistory.status, status));
    if (currency.length > 0)
        conditions.push(eq(matchesHistory.currency, currency));
    if (playerId.length > 0)
        conditions.push(or(eq(matchesHistory.player1Id, playerId), eq(matchesHistory.player2Id, playerId)));
    if (fromDate.length > 0 && !isNaN(Date.parse(fromDate)))
        conditions.push(gte(matchesHistory.createdAt, new Date(fromDate)));
    if (toDate.length > 0 && !isNaN(Date.parse(toDate)))
        conditions.push(lte(matchesHistory.createdAt, new Date(toDate)));
    const whereCondition = conditions.length > 0 ? and(...conditions) : undefined;
...
adminRouter.get("/ledger", requirePermission("WALLET_VIEW"), async (req, res) => {
    ...
    const conditions = [];
    if (userId.length > 0)
        conditions.push(eq(ledgerEntries.userId, userId));
    if (currency.length > 0 && (currency === "COINS" || currency === "DIAMONDS"))
        conditions.push(eq(ledgerEntries.currency, currency));
    const whereCondition = conditions.length > 0 ? and(...conditions) : undefined;
    ...
    try {
        const result = await db.transaction(async (tx) => {
            ...
            const existingReversal = await tx.query.adminAuditLogs.findFirst({
                where: and(eq(adminAuditLogs.action, "ADMIN_REVERSE_LEDGER_ENTRY"), eq(adminAuditLogs.targetId, originalLedgerId)),
            });
            if (existingReversal) {
