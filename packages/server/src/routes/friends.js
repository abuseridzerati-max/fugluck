...
friendsRouter.post("/request", socialLimiter, async (req, res) => {
    ...
    const existing = await db.query.friendships.findFirst({
        where: or(and(eq(friendships.requesterId, me), eq(friendships.addresseeId, target.id)), and(eq(friendships.requesterId, target.id), eq(friendships.addresseeId, me))),
    });
    if (existing) {
