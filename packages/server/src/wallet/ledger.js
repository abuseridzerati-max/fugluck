import { SIGNUP_COIN_GRANT } from "@arcadeclash/shared";
...
export async function lockWalletUsers(client, userIds) {
...
export async function getBalances(userId, client = db) {
    const rows = await client
        .select({
        currency: ledgerEntries.currency,
        total: sql `coalesce(sum(${ledgerEntries.amount}), 0)::int`,
    })
        .from(ledgerEntries)
    ...
    for (const row of rows) {
        if (row.currency === "COINS")
            coins = Number(row.total);
        if (row.currency === "DIAMONDS")
            diamonds = Number(row.total);
    }
    return { coins, diamonds };
}
async function hasReason(userId, reason, client = db) {
...
function validatePositiveInteger(amount, label) {
...
export async function checkAndApplyMonthlyAllowance(userId) {
...
export async function grantDiamondsStub(userId, diamonds, packId) {
...
export async function escrowStake(userId, currency, amount, matchId) {
...
export async function escrowMatchStakes(client, player1Id, player2Id, currency, amount, matchId) {
    validatePositiveInteger(amount, "Stake amount");
    validateCurrency(currency);
    if (player1Id === player2Id)
        throw new Error("A player cannot escrow both sides of a match.");
    ...
    if (p1Available < amount)
        throw new Error(`Player 1 has insufficient ${currency} balance for escrow.`);
    if (p2Available < amount)
        throw new Error(`Player 2 has insufficient ${currency} balance for escrow.`);
...
export async function payoutWinner(winnerUserId, loserUserId, currency, stakeAmount, matchId, rakePercent) {
    validatePositiveInteger(stakeAmount, "Stake amount");
    validateCurrency(currency);
    await ensureMatchSettlementsTable();
    const effectiveRake = rakePercent !== undefined
        ? rakePercent
        : currency === "COINS"
            ? COINS_RAKE_PERCENT
            : DIAMONDS_RAKE_PERCENT;
    return await db.transaction(async (tx) => payoutWinnerInTransaction(tx, winnerUserId, loserUserId, currency, stakeAmount, matchId, effectiveRake));
}
export async function payoutWinnerInTransaction(client, winnerUserId, loserUserId, currency, stakeAmount, matchId, rakePercent) {
    validatePositiveInteger(stakeAmount, "Stake amount");
    validateCurrency(currency);
    const effectiveRake = rakePercent !== undefined ? rakePercent : currency === "COINS" ? COINS_RAKE_PERCENT : DIAMONDS_RAKE_PERCENT;
    ...
    if (existingSettlement) {
        ...
        if (existingSettlement.status !== "PAYOUT" ||
...
export async function refundMatchSettlement(player1Id, player2Id, currency, stakeAmount, matchId, status = "REFUND") {
    validatePositiveInteger(stakeAmount, "Refund amount");
    validateCurrency(currency);
    await ensureMatchSettlementsTable();
    return await db.transaction(async (tx) => refundMatchSettlementInTransaction(tx, player1Id, player2Id, currency, stakeAmount, matchId, status));
}
export async function refundMatchSettlementInTransaction(client, player1Id, player2Id, currency, stakeAmount, matchId, status = "REFUND") {
    validatePositiveInteger(stakeAmount, "Refund amount");
    validateCurrency(currency);
    ...
    if (existingSettlement) {
        console.warn(`[ledger] IDEMPOTENT NO-OP: Match ${matchId} has already been settled or voided in database.`);
        if (existingSettlement.status !== status ||
            existingSettlement.currency !== currency ||
            existingSettlement.stake !== stakeAmount ||
...
export async function refundStake(userId, currency, amount, matchId) {
