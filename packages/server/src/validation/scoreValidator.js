...
import { checkReplayRequestShape, MAX_REPLAY_TICKS, replayEngine, UnrecognizedActionError, } from "@arcadeclash/shared";
...
export function validateScore(input) {
    ...
    if (!adapter)
        return { verdict: "invalid", reason: "unknown_game" };
    if (!input.viewport ||
    ...
    if (shapeError)
        return { verdict: "invalid", reason: shapeError };
    let outcome;
    try {
...
function checkFreezeFrameViolation(gameId, inputLog, durationMs, finalTick) {
    const TICK_HZ = 60;
    for (let i = 1; i < inputLog.length; i++) {
        const prev = inputLog[i - 1];
        const cur = inputLog[i];
        if (prev.wallMs === undefined || cur.wallMs === undefined)
            continue;
        ...
        if (wallDeltaSec - tickDeltaSec > FREEZE_FRAME_THRESHOLD_SEC) {
            console.warn(`[scoreValidator] ${gameId}: freeze-frame detected between ticks ${prev.tick}-${cur.tick} — ` +
    ...
    if (actualSec - expectedSec > FREEZE_FRAME_THRESHOLD_SEC) {
        console.warn(`[scoreValidator] ${gameId}: whole-run freeze-frame detected — ${actualSec.toFixed(1)}s real time for ` +
