...
import { generateSeed, isValidGameId, removeFromQueue } from "./queue";
const INVITE_TTL_MS = 60_000;
const pendingById = new Map();
// At most one outbound invite per inviter socket.
const pendingByFromSocket = new Map();
async function areFriends(a, b) {
    const row = await db.query.friendships.findFirst({
        where: and(eq(friendships.status, "accepted"), or(and(eq(friendships.requesterId, a), eq(friendships.addresseeId, b)), and(eq(friendships.requesterId, b), eq(friendships.addresseeId, a)))),
    });
    return Boolean(row);
}
function clearInvite(invite, notifyFrom, reason) {
...
export function cancelInvitesForSocket(socket) {
...
export async function handleInviteFriend(socket, payload) {
    ...
    if (priorId) {
        ...
        if (prior)
            clearInvite(prior, false);
    }
    const inviteId = randomUUID();
    const timer = setTimeout(() => {
        const invite = pendingById.get(inviteId);
        if (invite)
            clearInvite(invite, true, "Invite expired.");
    }, INVITE_TTL_MS);
    const invite = {
        id: inviteId,
        from: socket,
...
export function handleRespondInvite(socket, payload) {
...
export function handleCancelInvite(socket, payload) {
    if (!payload || typeof payload.inviteId !== "string")
        return;
    const invite = pendingById.get(payload.inviteId);
    if (!invite || invite.from !== socket)
        return;
    const target = getOnlineSocket(invite.toUserId);
    clearInvite(invite, false);
    target?.emit("inviteRejected", { inviteId: payload.inviteId, reason: "Invite cancelled." });
}
const guestLinksByCode = new Map();
export function handleCreateGuestLink(socket, payload) {
...
export function handleJoinGuestLink(socket, payload) {
