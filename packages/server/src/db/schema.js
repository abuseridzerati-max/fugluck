...
export const matchSettlements = pgTable("match_settlements", {
    ...
    amountCheck: check("match_settlements_amount_check", sql `${t.stake} > 0 and ${t.winnerPayout} >= 0 and ${t.rakeFee} >= 0`),
    payoutShapeCheck: check("match_settlements_payout_shape_check", sql `(${t.status} = 'PAYOUT' and ${t.winnerId} is not null and ${t.loserId} is not null) or (${t.status} <> 'PAYOUT' and ${t.winnerId} is null and ${t.loserId} is null and ${t.winnerPayout} = 0 and ${t.rakeFee} = 0)`),
}));
