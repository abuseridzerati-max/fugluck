...
export async function checkAdminLockout(ipAddress) {
...
export async function recordFailedAdminLogin(ipAddress) {
    await ensureLockoutTable();
    const record = await db.query.adminLockoutAttempts.findFirst({
        where: eq(adminLockoutAttempts.ipAddress, ipAddress),
    });
...
export async function resetAdminLockout(ipAddress) {
    await ensureLockoutTable();
    await db
        .update(adminLockoutAttempts)
        .set({
